...
<div class="content d-flex justify-content-center align-items-center">
    <!-- Registration form -->
    <form action="{{route('users.update', $user->id)}}" method="POST" class="flex-fill" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="row">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="form-control-feedback form-control-feedback-start">
                                        <input type="text" name="name" class="form-control" placeholder="JohnDoe"
                                            required autocomplete="name" value="{{old('name', $user->name)}}" disabled
                                            readonly>
                            ...
                            <div class="col-lg-12">
                                <div class="form-group mb-1">
                                    ...
                                    <select multiple="multiple" class="form-control" name="roles[]"
                                        ...
                                        <option value="{{$role->id}}" @selected(in_array($role->id,
                                            $user_roles))>{{$role->name}}</option>
                                        @endforeach
                                    </select>
                                    <input type="hidden" id="selectedRoles" name="selectedRoles"
                                        value="{{ $user->roles->pluck('id')->toJson() }}">
                                    @error('roles')
                                    <span class="text-danger">{{$message}}</span>
                                    @enderror
                                </div>
                                <script>
                                    $(document).ready(function(){
                                        $('#roles').select2();
                                        // Update hidden input field when selection changes
                                    ...
                                    <div class="form-control-feedback form-control-feedback-start form-group mb-1">
                                        <div class="input-group mb-3" id="password">
                                            <input type="password"
                                                ...
                                                autocomplete="new-password" value="{{old('password')}}" disabled
                                                readonly>
